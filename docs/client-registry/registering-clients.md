# Complete DataTemplate Example (Sample Payload)

This section provides a full example of a DataTemplate payload combining multiple blocks into a single request.

It demonstrates how client information, visit details, and clinical data are structured together when submitting data to the HDU API.

This example reflects a typical real-world scenario where a health facility submits a complete patient visit record.

## Example: Full DataTemplate Payload
```json
{
 "templateDetails": {
   "code": "GENERAL",
...
This sample payload shows how different parts of the DataTemplate work together:

- Client Registration → via `demographicDetails`
- Visit Tracking → via `visitDetails`
- Clinical Documentation → via `clinicalInformation`
- Diagnosis Reporting → via `diagnosisDetails`
- Laboratory Integration → via `laboratoryRequestDetails`
- Treatment Information → via `medicationDetails`

All these blocks are submitted in a single API request, making the integration efficient and consistent.



## Key Takeaways

- A single DataTemplate can contain multiple data blocks.
- Each entry inside listGrid represents a single patient visit.
- Blocks can be included or omitted depending on the service being provided.
- The structure must always follow the standard DataTemplate format.



## Final Note

Developers are encouraged to use this example as a reference template when building their own payloads for integration.
