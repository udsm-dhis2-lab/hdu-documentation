# Submitting Data Templates

## Submitting a Complete Data Template Payload

This approach is used when submitting a full set of client and visit information to the HDU API. It is commonly applied during the first data submission for a client or when a complete clinical encounter needs to be reported.

The payload includes all required core blocks (such as facility details, demographic details, and visit details) together with any relevant clinical, laboratory, or program-specific blocks. The HDU API validates the payload, registers or updates the client, and processes the data according to the configured workflows.
...
* **Method:** `POST`  
* **Description:**  
  Submits data payloads to the HDU API for processing and storage.
...
* **Description:**  
  Endpoint used to submit data based on defined template structures.
...
* **Description:**  
  Requires authorized credentials for secure data submission.



### Implementation Notes

- Ensure payload structure matches the expected template definition.  
- Use HTTPS to protect sensitive health information during transmission.  
- Validate responses to confirm successful submission or identify errors.
...
```json
{
   ...
   "summary": [
       {
           "hcrCode": "HCR-F-250416-15021998",
           "success": true,
           "message": "Client created successfully",
           "identifiers": [
               {
                   "type": "MRN",
                   "value": "109601-5-89989889"
               },
               {
                   "type": "HCRCODE",
                   "value": "HCR-F-250416-15021998"
               },
               {
                   "type": "NIDA",
                   "value": "19980215-93930202-98"
               },
               {
                   "type": "INS001",
                   "value": "99028388219"
   ...
   "invalidClientsSummary": [
       {
           "identifier": "MRN=109601-5-89989889",
           "index": 0,
           "validationIssues": [
               "Field 'outcomeDetails.deathLocation': Death location must be provided when isAlive is false",
               "Field 'outcomeDetails.deathDate': Death date must be provided when isAlive is false"
           ]
       }
   ]
}
...
This approach is used when only specific parts of a client’s data have changed during the course of care. Instead of resending the full data template, the EMR/EHR system submits only the mandatory identifiers together with the updated blocks.

Trigger-based submissions are commonly used for events such as admissions, discharges, diagnosis updates, referrals, and laboratory results. This allows the HDU API to process real-time updates efficiently while maintaining complete longitudinal records.
...
* **Format:** `JSON`  
* **Description:**  
  Payload contains structured JSON blocks representing updated client data.
...
* **Method:** `POST`  
* **Description:**  
  Submits selected data blocks to update or append existing client records.
...
* **URL:** `/api/v1/hduApi/dataTemplates`  
* **Description:**  
  Endpoint used for both full and partial (trigger-based) data submissions.
...
* **Type:** `Basic Authentication`  
* **Description:**  
  Requires authorized system credentials for secure communication.



### Implementation Notes

- Only modified or newly available blocks should be included in trigger-based submissions.  
- Client and visit identifiers must remain consistent with the original payload.  
- Systems should log trigger events for troubleshooting purposes.
...
**Note 1:** If there is no data for a required field, zero (`0`) should be filled where applicable.

**Note 2:** When the referral block is filled, the HDU API will automatically send a referral notification to the specified referral hospital.
